...
mod tests {
    ...
    fn hash_reduction_is_concurrency_stable() {
        // §9a guard within #2's scope: the frozen wire-format byte
        // constants (the input-hash SCHEME prefix + per-field tags,
        // defined in identity.rs; the source-tree header in tree.rs) are
        // NOT touched by this commit (`git diff` proves it; the #9
        // determinism suite + #96 drift-guard are the standing guards —
        // and this comment deliberately does not quote those literals, so
        // it stays outside the D1 drift-guard's banned-token surface).
        // What #2 must additionally
