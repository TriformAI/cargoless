...
//! server never reaches into the watcher/analyzer/model/build modules.
...
use tf_proto::{ArtifactMeta, BuildResult, StateEvent};
...
///
/// A WASM dev build is several files (an HTML shell, a JS loader, the
/// `_bg.wasm` blob, assets). The CAS contract ([`tf_cas::ContentStore`])
/// stores one opaque blob per [`tf_proto::InputHash`], so a bundle is
/// serialized with [`Bundle::pack`] / [`Bundle::unpack`] — a deliberately
/// trivial length-prefixed framing, owned here and shared with the build/CAS
/// layer (it is additive to neither crate's public contract; it is how the
/// single CAS blob is interpreted).
...
mod tests {
    ...
    fn bundle_pack_roundtrips() {
        ...
        let packed = b.pack();
        assert_eq!(Bundle::unpack(&packed).unwrap(), b);
    }
