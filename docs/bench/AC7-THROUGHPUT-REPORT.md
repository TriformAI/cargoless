...
## 11.4 Model R v1 — MEASURED-not-extrapolated (v4 supersession)

**§11.1–11.3 above is the *Model-A* fleet curve** (N independent
per-worktree daemons, the architecture v0 shipped). It is retained
verbatim as the honest comparison baseline: Model-A is a clean linear
~1.5 GiB/daemon, **extrapolated** to ~30 GiB @ 20 agents. Model R v1
(repo-scoped daemon, one rust-analyzer LSP-overlay-multiplexed across
all worktrees of a workspace-cluster) replaces that extrapolation with
a **measured** curve on the real wired daemon (`cargoless serve
--repo`, the capstone-wire `servedrv` driver @ `427c52e`).

### Provenance / controls (the v4 instrument)

`bench/modelr-fleet.sh` @ `agent/bench-lead-15bench` (committed,
own-source). **Per-ref-isolated `CARGO_TARGET_DIR=/tmp/mrf-tgt-<sha>`**
(the ci-gate `74b04df` per-ref discipline applied to bench/ — kills the
shared-`/cache/target` stale-artifact class; the v1-of-#15 self-caught
bug was exactly that) + a **binary-mtime ≥ build-start provenance
guard** (never measure an unprovenanced artifact). RSS = Σ `VmRSS`
over the live `serve` pid + its **recursive `pgrep -P` descendant
subtree** (NEVER pgid — the §11-v1 setsid lesson). Mechanism +
RSS captured **concurrently in the same live daemon instance**
(eliminates cross-run reconciliation). RA-absence ⇒ explicit FAIL,
never a fabricated number. Fixture: `bench/fixture` (Leptos-class,
honest-size) git-replicated into one shared-`Cargo.toml`/`Cargo.lock`
cluster of N worktrees (the dominant tf-multiverse cluster shape).

### Result — measured, mechanism-verified (own-eyes)

| N | aggregate peak (MiB) | aggregate avg (MiB) | RA-LSP procs (`lsp=`) | proc-macro-srv |
|---:|---:|---:|---:|---:|
| 1  | 1068 | 1034 | **1** | 1 |
| 2  | 1015 | 980  | **1** | 1 |
| 4  | 1012 | 983  | **1** | 1 |
...
**Aggregate fleet RSS is FLAT at ~1 GiB across N=1→20** — it does *not*
scale with worktree count (N=20 ≈ N=1). The load-bearing **mechanism**
is own-eyes-verified, not inferred: every cell's full descendant
`PROCTREE` (captured concurrently with the RSS sample) shows exactly
**one** `…/bin/rust-analyzer` LSP (`lsp=1` *constant* across all N,
*not* N-scaling) + its single `rust-analyzer-proc-macro-srv` (~95 MiB)
+ the ~3.8 MiB `serve` supervisor + a transient `cargo check` (the
authoritative verdict tier). One rust-analyzer multiplexed across the
whole cluster regardless of N — the Model R thesis, **measured**.

### The headline (replaces §11.2's Model-A compound-fit extrapolation)

| | per-fleet RSS @ 20 WT | basis |
|---|---:|---|
| Model A (per-WT daemon, §11.1) | ~30 GiB | measured-linear ~1.5 GiB × 20, **extrapolated** |
| **Model R (one multiplexed RA)** | **~1 GiB** | **MEASURED, flat N=1→20, mechanism-verified** |

⇒ **~19–30× fleet-RAM collapse, measured-not-extrapolated.** This is
the §11.2 "BORDERLINE / PROBABLY-YES / depends-on-workload" Model-A
compound-fit *superseded by a structural result*: Model R does not
*tune* the per-daemon footprint down toward a 16 GB fit — it removes
the per-worktree multiplication entirely.

### Honest caveats (load-bearing — carry into the launch narrative)

1. **The absolute (~0.9 GiB RA LSP) is fixture-dependent.** The
   bench fixture is Leptos-class honest-size but smaller than
   tf-multiverse; the load-bearing claim is the **flatness / one-
   multiplexed-RA structure**, NOT the absolute GiB. A larger
   workspace shifts the absolute (Tier-3 `--proc-macro disabled`
   etc. from §5/§10 still apply to that *one* RA) but **not** the
   flat-vs-N structure.
2. **Measured to N=20; beyond is a curve-shape projection.** The
   operator's real topology is 589/617 WT. The *measured* claim is
   N≤20-flat; "stays ~1 GiB at 589" is the projection the structure
   implies (one RA per cluster, independent of WT count) — stated as
   projection, not measurement.
3. **Closed-chain framing (NOT pure-unit-end-to-end).** Model R's
   verdict-correctness cores are *structurally proven* upstream;
   this bench is the **integration-validation half** of that closed
   chain (the live multiplexed daemon does behave — RAM AND one-RA
   structure). It is NOT, and is never claimed as, pure-unit-proven
   end-to-end.
4. **FIELD FINDING (disclosed, named).** The capstone-wire daemon
   does **not** `wait()`-reap its rust-analyzer child on shutdown:
   each `serve` exit leaves RA as a **zombie** (`stat=Z`, reparented
   to init, **0 RSS** — a process-table/PID accumulation under
   restart-churn, *NOT* a RAM leak — an earlier "~10 GiB OOM"
   inference was wrong and is retracted). Regression of the
   #3b/#44/#61/#128 + #13-leg-C ReapOnDrop discipline at the SIGTERM
   seam; routed to dev-fixer (#198), in fix-pipeline. **It does NOT
   impugn this measurement**: RSS is descendant-scoped (zombies
   reparent to init ⇒ structurally out of the measured subtree;
   verified, not asserted). Launch-relevant only for restart-churn
   process hygiene, not for the steady-state fleet-RAM thesis.
