...
The daemon's `--bind` control plane reuses the **same** hand-rolled HTTP server as the gate (`transport::http`). The only new route is `GET /app` (a JSON snapshot of every instance's phase/serving-sha/last-red/drain depth), exposed via one additive `VerdictService::app_report()` default method that returns `None` on the gate daemon — so `GET /app` on a gate is a `404` **byte-identical** to any unknown route (pinned by a guard test). `GET /readyz` reflects `AppServeState::ready()`, whose meaning is **"the daemon can accept and serve work"**: some instance is currently serving AND no *live* instance is failing to serve a green it owns. A never-green branch does not hold the pod un-ready; a red build never un-readies (the old green keeps serving); and — the liveness rule — an instance stops gating once its observable state has not changed for `readiness.stale_after_secs` (1h). "Live" is a fresh phase transition, **not** "this slot exists and is not green": an instance in active use re-arms its own stamp on every push, so a broken slot people care about keeps the probe red, while a slot abandoned for weeks ages out by rule rather than by an allowlist. Two abandoned instances (`merge`, `feature-x`) previously held `/readyz` at 503 indefinitely, delisting the healthy `dev`/`lane` with them. Aged-out instances are still reported on `/app` (`stale`, `idle_secs`, `readiness.stale_degraded`) — the probe answers "is it working", the report answers "what is not".
