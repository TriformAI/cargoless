...
- Branch-protection project checks are profile/config-driven unless the caller
  supplies a diff base. Use `cargoless checks run --profile <name> --base <ref>`
  in shared gate automation so YAML/docs-only changes can skip unrelated
  checks. A pure-YAML validation without CLI base pruning observed 54 project
  checks; that was fast enough, but it is still shared team infrastructure
  work.
